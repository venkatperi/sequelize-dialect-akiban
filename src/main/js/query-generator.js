...
module.exports = (function () {
  var QueryGenerator = {
    ...
    changeColumnQuery : function ( tableName, attributes ) {
      ...
      for ( var attributeName in attributes ) {
        ...
        if ( definition.indexOf( 'DEFAULT' ) > 0 ) {
          attrSql += Utils._.template( query )( {
            tableName : QueryGenerator.addQuotes( tableName ),
            query : QueryGenerator.addQuotes( attributeName ) + ' SET DEFAULT' +
    ...
    insertQuery : function ( tableName, attrValueHash ) {
      ...
      var replacements = {
        table : QueryGenerator.addQuotes( tableName ), attributes : Object.keys( attrValueHash ).map(function ( attr ) {
          return QueryGenerator.addQuotes( attr )
        } ).join( "," ), values : Utils._.values( attrValueHash ).map(function ( value ) {
    ...
    incrementQuery : function ( tableName, attrValueHash, where ) {
      ...
      for ( var key in attrValueHash ) {
        var value = attrValueHash[key]
        values.push( QueryGenerator.addQuotes( key ) + "=" + QueryGenerator.addQuotes( key ) + " + " +
            QueryGenerator.pgEscape( value ) )
      }
    ...
    hashToWhereConditions : function ( hash ) {
      ...
      for ( var key in hash ) {
        ...
        var _key = key.split( '.' ).map(function ( col ) {
              return QueryGenerator.addQuotes( col )
            } ).join( "." )
            , _value = null
        ...
        if ( Array.isArray( value ) ) {
          ...
          _value = value.join.split( '.' ).map(function ( col ) {
            return QueryGenerator.addQuotes( col )
          } ).join( "." )
    ...
    addQuotes : function ( s, quoteChar ) {
      ...
      return QueryGenerator.removeQuotes( s, quoteChar )
          .split( '.' )
          .map( function ( e ) {
            return quoteChar + String( e ) + quoteChar
          } )
          .join( '.' )
    },
      ...
      switch ( typeof val ) {
        ...
        case 'object':
          if ( Array.isArray( val ) ) {
            return 'ARRAY[' + val.map(function ( it ) {
              return QueryGenerator.pgEscape( it )
            } ).join( ',' ) + ']';
          }
      }
    ...
    pgEnum : function ( tableName, attr, dataType ) {
      ...
      return "DROP TYPE IF EXISTS " + enumName + "; CREATE TYPE " + enumName + " AS " +
    ...
    pgSqlDate : function ( dt ) {
      var date = [ dt.getUTCFullYear(), QueryGenerator.padInt( dt.getUTCMonth() + 1 ),
        QueryGenerator.padInt( dt.getUTCDate() ) ].join( '-' )
      var time = [ dt.getUTCHours(), QueryGenerator.padInt( dt.getUTCMinutes() ),
        QueryGenerator.padInt( dt.getUTCSeconds() )].join( ':' )
    ...
    pgDataTypeMapping : function ( tableName, attr, dataType ) {
      ...
      if ( dataType.match( /^ENUM\(/ ) ) {
        dataType =
            dataType.replace( /^ENUM\(.+\)/, QueryGenerator.pgEscapeAndQuote( "enum_" + tableName + "_" + attr ) )
      }
  ...
  return Utils._.extend( Utils._.clone( require( "sequelize" ).Dialects['query-generator'] ), QueryGenerator )
})();
